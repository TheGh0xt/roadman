...
# Primary source folder containing the user's PDF documents
ROADMAN_DATA_DIR = Path(r"c:\Users\PC\Downloads\ROADMAN!")
CORPUS_OUTPUT_FILE = Path(__file__).resolve().parent.parent / "data" / "highway_code.json"

def parse_pdf_documents() -> List[Dict[str, Any]]:
    """
    Parses PDF documents from the ROADMAN! directory and converts them into
    structured legal and road safety rules for RAG vector index.
    """
    extracted_laws = []

    if not ROADMAN_DATA_DIR.exists():
        print(f"[!] Warning: Roadman data directory not found at {ROADMAN_DATA_DIR}")
        return extracted_laws

    pdf_files = list(ROADMAN_DATA_DIR.glob("*.pdf")) + list(ROADMAN_DATA_DIR.glob("*.PDF"))
    print(f"[+] Found {len(pdf_files)} PDF documents in {ROADMAN_DATA_DIR}: {[f.name for f in pdf_files]}")
